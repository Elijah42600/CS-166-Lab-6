...
public class EmbeddedSQL {
   ...
   public static void Query3(EmbeddedSQL esql){
      try{
         String query = "SELECT sname, COUNT(*) FROM Catalog c INNER JOIN Suppliers s ON c.sid = s.sid INNER JOIN Parts p ON c.pid = p.pid GROUP BY sname HAVING EVERY (p.color = 'Green');";
   ...
   public static void Query4(EmbeddedSQL esql){
      try{
         String query = "SELECT s.sname, MAX(c.cost) FROM Catalog c INNER JOIN Suppliers s ON c.sid = s.sid INNER JOIN Parts p ON c.pid = p.pid GROUP BY s.sname
 HAVING ANY (P.color = ’Green’) AND ANY (P.color = ’Red’);";
   ...
   public static void Query5(EmbeddedSQL esql){
      try{
         String query = "SELECT p.pname FROM Parts p INNER JOIN Catalog c ON p.pid = c.pid WHERE c.cost < ";
         System.out.print("\tEnter cost: $");
         String input = in.readLine();
         query += input;
   ...
   public static void Query6(EmbeddedSQL esql){
      try{
         String query = "SELECT s.address FROM Suppliers INNER JOIN Catalog c ON s.sid = c.sid INNER JOIN Parts p ON c.pid = p.pid WHERE p.pname = ";
